...
var _ = Describe("CloudFormation template builder API", func() {
	...
	Describe("UserData - Ubuntu1804", func() {
		...
		It("should have packages, scripts and commands in cloud-config", func() {
			...
			kubeletSnapConf := getFile(c, "/etc/eksctl/kubelet.snap.conf")
			Expect(kubeletSnapConf).ToNot(BeNil())
			Expect(kubeletSnapConf.Permissions).To(Equal("0644"))
			checkAsset("kubelet.snap.conf", kubeletSnapConf.Content)
